...
export default function EventDetailsScreen({ route, onNavigate }) {
  const { eventId, returnTo } = route?.params || {};
  ...
  const checkFavoriteStatus = async () => {
    if (eventId) {
      const favorited = await checkIsFavorite('event', eventId);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.headerButton} onPress={() => {
          if (returnTo === 'favorites' && route?.params?.favoriteTab) {
            onNavigate('favorites', { favoriteTab: route.params.favoriteTab });
          } else {
            onNavigate(returnTo || 'events');
          }
        }}>
