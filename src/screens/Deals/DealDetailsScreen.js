...
export default function DealDetailsScreen({ onNavigate, dealId, authParams }) {
  ...
  const checkFavoriteStatus = async () => {
    if (dealId) {
      const favorited = await checkIsFavorite('deal', dealId);
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.headerButton} onPress={() => {
          if (authParams?.returnTo === 'favorites' && authParams?.favoriteTab) {
            ...
            onNavigate(authParams?.returnTo || 'testdeals');
          }
        }}>
